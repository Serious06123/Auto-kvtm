...
const { findCoordinates, findNumbersArray } = require('./image')
const { logErrMsg } = require('../service/log')

const MIN = -1
const MAX = 1
const MAX_RETRY = 3
const TAP_HOLD_TIME = 50
...
class Driver {
    ...
    startRecordingScreen = async () => {
        if (!SupportRecordVideo) return
        await this.driver.startRecordingScreen({ timeLimit: 30 * 60 })
    }

    stopRecordingScreen = async (key) => {
        if (!SupportRecordVideo) return
        const path = resolve(__dirname, `../../screen/${this.deviceId}_${key}.mp4`)
        try {
    ...
    getX = (x) => Math.round((this.width * x) / 100.0) + getRandomInt(MIN, MAX)
    getY = (y) => Math.round((this.height * y) / 100.0) + getRandomInt(MIN, MAX)
    ...
    action = async (points) => {
        ...
        for (let i = 1; i < points.length - 1; i++) {
            ...
            actionChain = actionChain.move({ duration: getSafeDuration(duration), x: this.getX(x), y: this.getY(y) })
        }
...
const KeyCode = { HOME: 3, BACK: 4 }

const SwipeDirection = { UP: 'up', DOWN: 'down', LEFT: 'left', RIGHT: 'right' }
